...
from poke_env.core import Agent
from poke_env.policy2 import EpsGreedyQPolicy, GreedyQPolicy
from rl.util import *
...
class DQNAgent(AbstractDQNAgent):
	...
	def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=False, enable_dueling_network=False,
		...
		#if model.output._shape != (None, self.nb_actions):
	...
	def forward(self, observation, action_mask=None):
		...
		if self.training:
			action = self.policy.select_action(q_values=q_values, action_mask=action_mask)
		else:
			action = self.test_policy.select_action(q_values=q_values, action_mask=action_mask)
